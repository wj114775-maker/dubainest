...
const initialForm = { user_id: "", organisation_type: "partner_agency", organisation_id: "", membership_type: "agency_admin", status: "active", assignment_scope: "", notes: "" };

export default function AdminMembershipCard({ onCreate, membership, onCancel }) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      organisation_type: membership.organisation_type || "partner_agency",
      organisation_id: membership.organisation_id || "",
      membership_type: membership.membership_type || "agency_admin",
  ...
  return (
    <Card className="rounded-[2rem] border-white/10 bg-card/80">
      <CardHeader><CardTitle>{membership ? "Edit organisation membership" : "Create organisation membership"}</CardTitle></CardHeader>
      <CardContent className="grid gap-4 md:grid-cols-2">
        ...
        <div className="md:col-span-2 flex gap-3">
          <Button onClick={() => onCreate(form)}>{membership ? "Save membership" : "Create membership"}</Button>
          {membership ? <Button variant="outline" onClick={onCancel}>Cancel</Button> : null}
        </div>
      </CardContent>
    </Card>
  );
