    ...
    <Card className="rounded-[2rem] border-white/10 bg-card/80">
      ...
      <CardContent className="space-y-4 text-sm">
        <div className="rounded-2xl border border-white/10 p-4">
          ...
          <p className="mt-1 text-muted-foreground">{activeWindow ? [activeWindow.status, activeWindow.lock_reason, activeWindow.protected_until].filter(Boolean).join(" · ") : "No active protection window."}</p>
        </div>
        <div className="rounded-2xl border border-white/10 p-4">
