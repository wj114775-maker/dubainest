...
function ExternalLinkButton({ href, label, icon: Icon }) {
  return (
    <a
      ...
      rel="noreferrer"
      aria-label={label}
      className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-950 bg-slate-950 text-white shadow-[0_14px_28px_rgba(15,23,42,0.14)] transition hover:-translate-y-0.5 hover:border-slate-800 hover:bg-slate-800"
    >
          ...
          <div className="grid gap-4 rounded-[1.6rem] border border-slate-200 bg-white px-4 py-4 shadow-sm shadow-black/5 lg:grid-cols-[minmax(0,1fr)_auto_auto] lg:items-center lg:px-5">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              {socialLinks.map((item) => (
                <ExternalLinkButton key={item.label} href={item.href} label={item.label} icon={item.icon} />
              ))}
            </div>
