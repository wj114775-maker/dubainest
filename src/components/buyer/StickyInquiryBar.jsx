...
export default function StickyInquiryBar() {
  ...
  return (
    <>
      <div
        ref={widgetRef}
        className="group pointer-events-none fixed bottom-24 right-4 z-40 md:bottom-6 md:right-6 lg:bottom-auto lg:right-6 lg:top-1/2 lg:-translate-y-1/2"
        onMouseEnter={() => {
      ...
      >
        <div className="pointer-events-auto relative flex items-center justify-end">
          ...
          <div
            className={cn(
              "absolute right-0 z-10 w-[calc(100vw-2rem)] max-w-[24rem] transition duration-200 lg:right-20 lg:w-[23rem]",
              isMobile
                ? showPanel
                  ? "bottom-16 pointer-events-auto visible translate-y-0 opacity-100"
                  : "bottom-16 pointer-events-none invisible translate-y-4 opacity-0"
                : showPanel
                  ? "top-1/2 pointer-events-auto visible -translate-y-1/2 translate-x-0 opacity-100"
                  : "top-1/2 pointer-events-none invisible -translate-y-1/2 translate-x-4 opacity-0"
            )}
          >
          ...
          {!showPanel && !isMobile ? (
            <div className="pointer-events-none absolute right-20 top-1/2 -translate-y-1/2 rounded-full bg-slate-950 px-3 py-2 text-xs font-medium text-white opacity-0 shadow-[0_12px_24px_rgba(15,23,42,0.18)] transition duration-200 group-hover:opacity-100">
              Need help?
            </div>
          ) : null}
